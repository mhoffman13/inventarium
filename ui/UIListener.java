...
public class UIListener implements Initializable {
	...
	private Parent view;// variable stores different views of application ex.
						// view all
	...
	@FXML
	private TextField vendorNumber;
	@FXML
	private TextField vendorDescription;
	@FXML
	...
	private void handleVendorButton(ActionEvent event) {
		...
		Vendor vendor = null;
		try {
			vendor = new Vendor(123, vendorName.getText(),
					vendorDescription.getText(), vendorContact.getText(),
					vendorPhone.getText(), null, null, EntityStatus.ACTIVE);
		} catch (Exception e) {
			System.out.println(e);
			e.printStackTrace();
		}
		if (vendor != null && addVendor.addEditEntity(vendor)) {
			System.out.println("Vendor add for " + vendorName.getText()
					+ " successful");
		} else {
	...
	void addVendor(ActionEvent event) {
		PageNavigator.loadPages(PageNavigator.ADD_VENDOR_VIEW);
	}

	@FXML
	void inventoryAll(ActionEvent event) {
		PageNavigator.loadPages(PageNavigator.INVENTORY_VIEW);
	}
